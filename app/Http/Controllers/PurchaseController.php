    ...
    public function counting(Request $request)
    {
        ...
        $counts = app(Pipeline::class)
            ...
            ->thenReturn()->selectRaw("
                COUNT(*) as recieved,
                SUM(CASE 
                    WHEN (tab = " . Purchase::TAB_VERIFIED . " AND due_date < NOW()) THEN sub_total ELSE 0 END) +
                SUM(CASE 
                    WHEN (tab = " . Purchase::TAB_PAYMENT_REQUEST . " AND purchase_status_id = " . PurchaseStatus::OVERDUE . ") THEN sub_total ELSE 0 END) as over_due,
